...
async function init() {
  ...
  try {
    const response = await fetch("./commands/index.json?v=35", { cache: "no-store" });
...
async function selectCommand(id) {
  ...
  try {
    const response = await fetch(`./commands/${encodeURIComponent(item.file)}?v=35`, {
      cache: "no-store"
    });
...
function validate(values) {
  ...
  (schema.fields || []).forEach(f => {
    ...
    // Every parameter is optional. Validate only when the user supplied a value.
    if (!value) return;
...
function buildCommand() {
  ...
  // Build only the parameters the user actually filled.
  // This keeps optional Huawei parameters out of the generated command.
  const params = (schema.fields || [])
    .filter(f => values[f.id])
    .map(f => {
      const format = f.output || `${f.id}={value}`;
      return format.replace(/\{value\}/g, values[f.id]);
    })
    .join(",");

  const neNameRaw = values.Remark || "";
  const neName = neNameRaw
    ? (neNameRaw.startsWith("{") && neNameRaw.endsWith("}")
        ? neNameRaw
        : `{${neNameRaw}}`)
    : "";

  const template = String(schema.template || "COMMAND:{params};{Remark}");
  const result = template
    .replace(/\{params\}/g, params)
    .replace(/\{Remark\}/g, neName);

  output.textContent = result;
  statusEl.textContent = params || values.Remark ? "Generated" : "Generated (empty parameters)";
  return result;
}
...
commandSearch.addEventListener("focus", event => {
  const value = event.target.value.trim();
  if (value) renderCommandResults(value);
});
